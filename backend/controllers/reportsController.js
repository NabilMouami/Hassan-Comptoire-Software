 ...
 */
const buildDateRange = (startDate, endDate, column = "date_creation") => {
  ...
  try {
    ...
    const factureStats = await Facture.findAll({
      ...
      attributes: [
        ...
        [fn("SUM", col("montant_ht")), "total_ht"],
        [fn("SUM", col("montant_ttc")), "total_ttc"],
        [fn("SUM", col("montant_paye")), "total_paye"],
        [fn("SUM", col("montant_restant")), "total_restant"],
      ],
      group: ["status"],
      raw: true,
    });
    ...
    res.json({
      ...
      factures: {
        totals: {
          ...
          montant_ht: factureTotals.total_ht.toFixed(2),
          montant_ttc: factureTotals.total_ttc.toFixed(2),
          montant_paye: factureTotals.total_paye.toFixed(2),
          montant_restant: factureTotals.total_restant.toFixed(2),
          taux_recouvrement:
      ...
      bon_livraisons: {
        totals: {
          ...
          montant_ht: blTotals.total_ht.toFixed(2),
          montant_ttc: blTotals.total_ttc.toFixed(2),
        },
        by_status: blStats,
      },
      advancements: {
...
const getRevenueOverTime = async (req, res) => {
  try {
    ...
    const factureRevenue = await Facture.findAll({
      where: { ...dateFilter, status: { [Op.ne]: "annulée" } },
      attributes: [
        [fn("DATE_FORMAT", col("date_creation"), fmt), "period"],
        [fn("COUNT", col("id")), "nb_factures"],
        [fn("SUM", col("montant_ht")), "total_ht"],
        [fn("SUM", col("montant_ttc")), "total_ttc"],
        [fn("SUM", col("montant_paye")), "total_paye"],
      ],
      group: [fn("DATE_FORMAT", col("date_creation"), fmt)],
      order: [[fn("DATE_FORMAT", col("date_creation"), fmt), "ASC"]],
      raw: true,
    });
  ...
  try {
    ...
    const clientStats = await Facture.findAll({
      ...
      attributes: [
        ...
        [fn("SUM", col("montant_paye")), "total_paye"],
        [fn("SUM", col("montant_restant")), "total_restant"],
        [fn("AVG", col("montant_ttc")), "avg_facture"],
        [fn("MAX", col("date_creation")), "last_facture_date"],
      ],
      include: [
        {
          model: Client,
    ...
    const clientBLStats = await BonLivraison.findAll({
      ...
      attributes: [
        "client_id",
        [fn("COUNT", col("id")), "nb_bls"],
        [fn("SUM", col("montant_ttc")), "total_bl_ttc"],
      ],
      group: ["client_id"],
      raw: true,
    });
...
const getTVAReport = async (req, res) => {
  try {
    ...
    const tvaByPeriod = await Facture.findAll({
      ...
      attributes: [
        [fn("DATE_FORMAT", col("date_creation"), fmt), "period"],
        [fn("SUM", col("montant_ht")), "base_ht"],
        [fn("SUM", col("montant_tva")), "total_tva"],
        [fn("SUM", col("montant_ttc")), "total_ttc"],
        [fn("COUNT", col("id")), "nb_factures"],
      ],
      group: [fn("DATE_FORMAT", col("date_creation"), fmt)],
...
const getBLConversionReport = async (req, res) => {
  try {
    ...
    const convertedBLs = await BonLivraison.count({
      where: { ...dateFilter, is_facture: true },
    });
    ...
    const totalValueConverted = await BonLivraison.findOne({
      where: { ...dateFilter, is_facture: true },
      attributes: [[fn("SUM", col("montant_ttc")), "total"]],
      raw: true,
    });
